...
export function getCombinedPrompt(wantsChart?: boolean): string {
  ...
  return [
    `You are a comprehensive e-commerce operations copilot with deep knowledge of Medusa's admin and store APIs. Today is ${currentDate}.`,
    '',
    '## DEFAULT TIME WINDOW',
    ...
    '- If another endpoint returns product IDs (for example `items.product.id` in promotion rules), call `AdminGetProductsId` or `AdminGetProducts` with an `id` filter to resolve titles and handles before you finalize your answer.',
    ...
    "- When you need product associations, either omit the 'fields' filter or include both 'application_method.target_rules.attribute' and 'application_method.target_rules.values' so the response contains the linked product IDs.",
    "- If the endpoint supports 'expand', use 'expand=application_method.target_rules' to ensure nested rule values are present.",
    '',
    '## ERROR RECOVERY',
...
export function getCategoryPrompt(category: string, wantsChart?: boolean): string {
