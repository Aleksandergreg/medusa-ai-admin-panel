...
export function createPromotionAnalyticsService(http: Http): {
        ...
        for (const order of orders) {
            ...
            if (promotion_code) {
                const codeMatches = promotion_codes.some(
                    (code) =>
                        code.toLowerCase() === promotion_code.toLowerCase()
                );
                if (!codeMatches) {
    ...
    async function getPromotionProductStats(
        ...
        for (const order of ordersWithPromotions) {
            const orderPromotionCodes = promotion_code
                ? order.promotion_codes.filter(
                      (code) => code === promotion_code
                  )
                : order.promotion_codes;

            if (orderPromotionCodes.length === 0) {
                continue;
            }
            ...
            for (const item of order.items) {
                ...
                stats.discount_amount +=
                    order.discount_total / order.items.length; // Distribute discount evenly
            }
        }
    ...
    async function getPromotionsSummary(
            ...
            for (const code of order.promotion_codes) {
                if (!promotionSummaries.has(code)) {
                    promotionSummaries.set(code, {
                        promotion_code: code,
                        promotion_id:
                            order.promotion_ids.find((id) => id) || null,
                        total_orders: 0,
        ...
        for (const [code, summary] of promotionSummaries) {
            const productStats = await getPromotionProductStats(
                start,
                end,
                code
            );
