    ...
    handler: async (input: Record<string, unknown>): Promise<unknown> => {
      ...
      const parseDurationToMinutes = (v: any): number | undefined => {
        ...
        if (short) {
          ...
          if (unit === "d") return Math.round(num * 1440);
        }
        // e.g., "1 minute", "2 hours", "3 days", "1 min"
        const long = s.match(/^(\d+(?:\.\d+)?)\s*(minute|minutes|min|m|hour|hours|hr|h|day|days|d)$/i);
        if (long) {
          const num = parseFloat(long[1]);
          const unit = long[2].toLowerCase();
          if (["minute", "minutes", "min", "m"].includes(unit)) return Math.round(num);
          if (["hour", "hours", "hr", "h"].includes(unit)) return Math.round(num * 60);
          if (["day", "days", "d"].includes(unit)) return Math.round(num * 1440);
        }
        // numeric string -> minutes
        const asInt = coerceInt(s);
        if (typeof asInt === "number") return Math.max(0, asInt);
        return undefined;
      };
      ...
      // limit/offset
      const limit = Math.max(1, Math.min(100, coerceInt(getAlias(input, ["limit", "take", "top", "page_size"])) ?? 50));
